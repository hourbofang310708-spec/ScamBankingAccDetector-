...
def clean_text(text: str) -> str:
  """Remove tabs or newlines that might break TSV structure."""
  return text.replace("\t", " ").replace("\n", " ")


def generate_dataset(filename: str, count_per_class: int):
  dataset = []
  ...
  for _ in range(count_per_class):
    ...
    dataset.append(f"spam\t{clean_text(msg)}")
  ...
  for _ in range(count_per_class):
    ...
    dataset.append(f"ham\t{clean_text(msg)}")

  random.shuffle(dataset)

  output_path = Path(filename)
  output_path.write_text("\n".join(dataset), encoding="utf-8")
  print(f"Successfully generated {len(dataset)} messages in {output_path}!")


if __name__ == "__main__":
  parser = argparse.ArgumentParser(
      description="Generate synthetic financial text classification dataset."
  )
  parser.add_argument(
      "--output",
      default="bank_bigdata.txt",
  ...
  parser.add_argument(
      ...
      type=int,
      default=2500,
      help="Number of samples per class (spam/ham)",
  )
